...
boton.addEventListener("click", ()=>{
  html = "";
  let contador = 0;
  if(isNaN(parseInt(desde.value)) || isNaN(parseInt(hasta.value)) || isNaN(parseInt(cuartos.value))){
    alert("Faltan campos por llenar");
    return;
  }
  console.log(desde.value)
  for (propiedad of propiedadesJSON) {
    if (parseInt(desde.value) <= propiedad.m && parseInt(hasta.value)>= propiedad.m && propiedad.rooms=== parseInt(cuartos.value) ){
    html += `
      <div class="propiedad">
        <div
          class="img"
